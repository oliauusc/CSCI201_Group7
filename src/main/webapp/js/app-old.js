...
const CONFIG = {
    ...
    API_BASE_URL: '/foodlocator/api', // Include context path
    REVIEWS_PER_PAGE: 10
};
...
async function loadPlaces() {
    try {
        ...
        if (data.success && data.data) {
            allPlaces = data.data.map(location => ({
                id: location.locationID,
                name: location.name,
                lat: location.lat,
                lng: location.lng,
                rating: location.rating || 0,
                description: location.description || location.category,
                tags: location.tags || [],
                distance: location.distance || 0
            }));
            console.log('Loaded places from API:', allPlaces);
        } else {
        ...
        console.log('Places loaded and displayed');
        showLoading(false);
    } catch (error) {
...
function displayPlaces(places) {
    ...
    console.log('placesList element:', placesList);
    console.log('places to display:', places);
    
    if (!placesList) {
        console.error('ERROR: placesList element not found!');
        return;
    }
    ...
    const html = sortedPlaces.map((place, index) => `
    ...
    console.log('Places HTML set, count:', sortedPlaces.length);
...
async function loadAllReviews(locationId, page = 1, sortBy = 'recent', filters = []) {
    try {
        ...
        const response = await fetch(`${CONFIG.API_BASE_URL}/reviews/${locationId}?page=${page}&pageSize=${CONFIG.REVIEWS_PER_PAGE}&sortBy=${sortBy}`, { credentials: 'include' });
        const result = await response.json();
        
        if (result.success && result.data && result.data.reviews) {
            let allReviews = result.data.reviews;
            
            // Apply tag filters
            if (filters.length > 0) {
                allReviews = allReviews.filter(review =>
                    filters.some(tag => review.tags && review.tags.includes(tag))
                );
            }
            ...
            updatePagination();
            updateReviewsCount(result.data.totalReviews || allReviews.length);
        } else {
            displayAllReviews([]);
...
async function handleReviewSubmit(e) {
    ...
    console.log('Form values:', { rating, title, body, currentLocation });
...
function setupEventListeners() {
    ...
    document.getElementById('writeReviewBtn').addEventListener('click', () => {
        ...
        // Redirect to write review page with location info
        const location = currentLocation;
        window.location.href = `/foodlocator/jsp/write-review.jsp?locationId=${location.id}&locationName=${encodeURIComponent(location.name)}`;
    });
